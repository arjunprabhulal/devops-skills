...
`.agents/skills/<skill>/` layout, which is prebuilt in this repo; copy a folder into
...
## Why are there two copies of every skill in the repository?
...
`.agents/skills/<skill>/` is a flat mirror that Google Antigravity discovers, generated by
`scripts/build-antigravity.py`. Skill names are globally unique, so flattening is lossless. Edit
the canonical copy only — the build script deletes and regenerates `.agents/skills/` — then re-run
it. `check-skills.py` fails if the two trees drift apart.
