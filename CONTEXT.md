...
Why this repository is shaped the way it is. `README.md` describes what the collection contains and
how to install it. `AGENTS.md` describes how to work inside it. This file records the reasoning
behind the structure, so that changes extend the design instead of eroding it.
...
incomplete, and assumes the reader can follow a name to the neighbouring skill.
...
failure modes of each, not a module that can be applied unread. The collection is optimised for
...
## Two layouts, one of them generated

Claude Code discovers skills from the category-nested tree listed in `.claude-plugin/plugin.json`:
`skills/<category>/<skill>/SKILL.md`. Google Antigravity discovers them from a flat
`.agents/skills/<skill>/SKILL.md` layout. Both consume the same file format, so the only difference
is directory shape. The nested tree is canonical because categories are how the collection is
browsed, reviewed, and kept balanced — 15 groups of 3 to 9 skills. The flat tree carries no
information the nested one lacks, so hand-maintaining both would mean two copies of 88 files with
...
byte against its canonical copy and flags stale directories. Hand edits under `.agents/` are lost
on the next build.
...
Organisation-specific procedure belongs in organisation-specific documents, and the skills are
